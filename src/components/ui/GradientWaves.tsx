...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    // Track the pointer on the window instead of the canvas: the waves usually
    // render as a background layer behind overlaying content (e.g. the footer
    // body), which would otherwise swallow every pointermove before it can
    // reach the canvas and disable the parallax interaction.
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (x < 0 || x > 1 || y < 0 || y > 1) {
        // Pointer left the component bounds — ease back to the centered rest view.
        targetMouse[0] = 0.5;
        targetMouse[1] = 0.5;
        return;
      }
