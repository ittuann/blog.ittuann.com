...
description: 使用 Rclone  对文件夹进行简易每日备份，以及使用 Restic 备份工具。
pubDate: 2024-10-21
tags: ["Linux"]
category: ["Tech"]
---
...
Sync 同步。Rclone 它更多的是作为云同步工具。没有 Snapshots 快照版本管理功能。

简易备份的方式有 2 种，一种是用增量同步（只上传改变的文件）待备份文件夹，另一种是先将目标文件夹压缩为 zip 再上传压缩文件夹。
...
然后根据提示，完成配置 remote 的 webdav 设置。
...
没有选择用 sftp 而是用了 webdav，是处于安全性的考虑。

rclone 支持 3 种身份验证方法：Password、Key file、ssh-agent

用 rclone 本身加载 Key file 时，仅能加载未加密（没有密码）的 key。而加载加密 OpenSSH 密钥的唯一方法是使用 ssh-agent。 <https://rclone.org/sftp/#ssh-authentication>

因为 rclone 不能在工具内部安全的保存密钥的密码，而创建未加密的密钥并不安全，此外为了一个全自动定时脚本而让 ssh-agent 始终存储密钥也不够安全。

## 压缩备份同步脚本

步骤是先将目标文件夹压缩为带密码的 zip 文件，再将整个压缩文件夹复制到远程。
...
开源项目，是 Web 页面对操作更友好。但 CLI 命令行的支持在官方文档中就写了一页，感觉软件会更侧重于 GUI 而不是用于服务器的 CLI。项目有 Docker 镜像可以直接拉。
